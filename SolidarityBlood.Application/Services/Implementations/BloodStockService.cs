...
namespace SolidarityBlood.Application.Services.Implementations
{
    public class BloodStockService : IBloodStockService
    {
        ...
        {
            var stock = _dbcontext.BloodStocks.SingleOrDefault(d => d.Id == id);

            _dbcontext.BloodStocks.Remove(stock);
            _dbcontext.SaveChanges();
        }

        public List<GetAllBloodStockDTO> GetAll()
        {
            var bloodStock = _dbcontext.BloodStocks.Select(bl => new GetAllBloodStockDTO(
                bl.Id,
                bl.BloodType,
                bl.RHFactor,
                bl.QuantityMl
        ...
        {
            var stock = _dbcontext.BloodStocks.SingleOrDefault(bs => bs.Id == id);
